...
```python
class UserService:
    def check_user_verification(self, user_id: str) -> dict:
        """Returns: {'isVerified': bool, 'isInboxCreated': bool, 'isGmailConnected': bool}"""
    
    def get_user_profile(self, user_id: str) -> dict:
        """Returns: User profile with subscription, inbox, Gmail accounts"""
    
    def get_complete_user_data(self, user_id: str) -> dict:
        """Returns: Complete user data including emails, newsletters, stats, etc."""
    
    def create_inbox(self, user_id: str, username: str) -> dict:
        """Returns: {'success': bool, 'email': str, 'message': str}"""
    
    def get_dashboard_data(self, user_id: str) -> dict:
        """Returns: Dashboard statistics (emails, reading, subscriptions, storage)"""
```
...
class EmailService:
    ...
    def get_email(self, email_id: str) -> dict:
        """Returns: Email details with content"""
    
    def toggle_favorite(self, email_id: str) -> dict:
        """Returns: {'is_favorite': bool, 'message': str}"""
...
class AuthService:
    ...
    def verify_otp(
        ...
        """Returns: {'access': str, 'refresh': str, 'user': dict, 'isNewUser': bool}"""
    
    def refresh_token(self, refresh_token: str) -> dict:
        """Returns: {'access': str, 'expiresAt': str}"""
```
...
# Get user profile
profile = user_service.get_user_profile(user_id="123e4567-e89b-12d3-a456-426614174000")
print(f"User: {profile['email']}")
print(f"Subscription: {profile['subscription']['plan']}")

# Create inbox
result = user_service.create_inbox(
    user_id="123e4567-e89b-12d3-a456-426614174000",
    username="johndoe"
)
print(f"Inbox created: {result['email']}")

# Get dashboard data
dashboard = user_service.get_dashboard_data(user_id="123e4567-e89b-12d3-a456-426614174000")
print(f"Unread emails: {dashboard['emails']['unread']}")
print(f"Reading streak: {dashboard['reading']['streak']}")
```
...
# Toggle favorite
result = email_service.toggle_favorite(email_id="email-uuid")
print(f"Favorite status: {result['is_favorite']}")

# Add highlight
highlight = email_service.add_highlight(
    email_id="email-uuid",
    text="Important point",
    note="Remember this"
)
print(f"Highlight created: {highlight['id']}")
```
...
# Send OTP
result = auth_service.send_otp("user@example.com")
print(result['message'])

# Verify OTP
tokens = auth_service.verify_otp(
...
print(f"New user: {tokens['isNewUser']}")

# Refresh token
new_tokens = auth_service.refresh_token(tokens['refresh'])
...
```python
from subscription_app.services import SubscriptionService
...
for plan in plans:
    print(f"{plan['name']}: ${plan['price']}/{plan['billingInterval']}")

# Get user subscription
subscription = subscription_service.get_user_subscription(user_id="user-uuid")
...
def create_inbox(self, user_id: str, username: str) -> dict:
    try:
        ...
        # Validate username
        if not self.check_inbox_availability(username)['available']:
            raise ValueError('Username not available')
        
        # Create inbox
        inbox = InboxEmail.objects.create(
            user=user,
            email=username
        )
...
def get_dashboard_data(self, user_id: str) -> dict:
    ...
    # Fetch data
    data = self._fetch_dashboard_data(user_id)
...
def send_otp(self, email: str) -> dict:
    ...
    try:
        ...
        # Send email
        self.nodemailer_service.send_otp(email, otp)
...
Services use database transactions for atomic operations:
```python
