...
graph TB
    ...
    subgraph "🌳 Bonsai.io Elasticsearch Cluster"
        Bonsai[Bonsai.io Cluster]
        
        subgraph "Global Search Index"
            NPIndex[newsletter_providers<br/>📰 Global Newsletter Directory]
        end
        
        subgraph "User-Level Search Indexes"
            EmailIndex[emails<br/>📧 User Email Search]
            NewsIndex[newsletters<br/>📬 User Subscriptions]
            PostIndex[newsletter_posts<br/>📄 Newsletter Posts]
            InboxIndex[inbox_emails<br/>📥 Inbox Emails]
        end
    end
...
sequenceDiagram
    ...
    alt Cache Hit (60-80% of queries)
        Cache-->>API: Return cached results (~10ms)
        API-->>Client: Search results
    else Cache Miss (20-40% of queries)
...
#### Cache Hit Path (60-80% of queries)
- **Response Time**: < 10ms
- **Source**: Redis cache
- **No External Calls**: Bypasses Bonsai.io

#### Cache Miss Path (20-40% of queries)
...
- **Bonsai.io API Calls**: Reduced by 60-80% via caching
